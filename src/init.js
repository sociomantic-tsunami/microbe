...
function _create( _el )
{
    ...
    for ( let i = 0, lenI = resultsRegex.length; i < lenI; i++ )
    {
    ...
    if ( typeof _tag === 'string' )
    {
        ...
        if ( _class )
        {
            ...
            for ( let i = 1, lenI = _class.length; i < lenI; i++ )
            {
 ...
 * @return  {Microbe}
*/
const init = Microbe.core.__init__ = function( _selector, _scope, _elements )
{
    if ( !_scope )
    {
    ...
    if ( _scope !== document )
    {
        ...
        if ( typeof _scope === 'string' && typeof _selector === 'string' )
        {
            if ( _selector.indexOf( ',' ) !== -1 || _scope.indexOf( ',' ) !== -1 )
            {
                ...
                for ( let i = 0, lenI = _scope.length; i < lenI; i++ )
                {
                    for ( let j = 0, lenJ = _selector.length; j < lenJ; j++ )
                    {
    ...
    if ( _selector.indexOf( ':' ) !== -1 )
    {
        ...
        for ( let i = 0, lenI = pseudo.length; i < lenI; i++ )
        {
